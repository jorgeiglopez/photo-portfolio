...
const useDiscovery = (albumName) => {
    ...
    useEffect(() => {
        ...
        projectStorage.ref(albumName).listAll().then(res => {
            if (res.items && res.items.length > 0) {
                res.items.forEach(item => {
                    item.getDownloadURL().then(url => {
                        ...
                        dynamoRef.doc(myPhoto.id).set(myPhoto, { merge: true });
                        accum.push(myPhoto)
                    })
                    item.getMetadata().then(meta => {
                        ...
                        dynamoRef.doc(myMeta.id).set(myMeta, { merge: true });
                    })
                })
            }
        });
...
export const useDiscoveryAll = () => {
    ...
    useEffect(() => {
        const albums = [];
        projectStorage.ref().listAll().then(res => {
            if(res.prefixes) {
